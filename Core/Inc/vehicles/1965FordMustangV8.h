...
uint32_t revVolumePercentage = 80; // Adjust the idle volume (usually = 100%, more also working, depending on sound)
uint32_t engineRevVolumePercentage = 80; // the engine volume will be throttle dependent (usually = 40%, never more than 100%!)
volatile const uint32_t revSwitchPoint = 10; // The rev sound is played instead of the idle sound above this point
volatile const uint32_t idleEndPoint = 500; // above this point, we have 100% rev and 0% idle sound volume (usually 500, min. 50 more than revSwitchPoint)
volatile const uint32_t idleVolumeProportionPercentage = 90; // The idle sound volume proportion (rest is rev proportion) below "revSwitchPoint" (about 90 - 100%, never more than 100)
#ifdef REV_SOUND
...
volatile int dieselKnockStartPoint = 50; // Volume will raise above this throttle input( usually 0, for "open pipe" exhaust about 250)
...
const bool INDICATOR_DIR = true; // adjust indicator direction with true or false
...
const bool doubleFlashBlueLight = true; // double flash blue lights if "true", "rotating" beacons if "false"
...
const bool pwmSoundTrigger = true; // horn triggered by RC PWM signal instead of constant high level signal, if "true"
...
const bool automatic = true; // false = linear rpm curve, true = automatic transmission with torque converter is simulated (use it, if you don't have a real shifting transmission)
#define NumberOfAutomaticGears 3 // <<------- Select 3, 4 or 6 gears!
const bool doubleClutch = false; // do not activate it at the same time as automatic!
const bool shiftingAutoThrottle = true; // For Tamiya 3 speed tansmission, throttle is altered for synchronizing, if "true"
